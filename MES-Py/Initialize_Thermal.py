...
#%% Electricity demand
Electricity_Demand = pd.read_csv('Inputs/Electric_Demand.csv', index_col=0) # Import electricity demand
Electricity_Demand = Electricity_Demand/1000      # Convert in kW
Electricity_Demand = round(Electricity_Demand, 1) 
...
def Initialize_Demand(model,i,t):
    ...
    '''
    return float(Electricity_Demand.iloc[t-1,i-1])
...
#%% PV output
PV_Energy = pd.read_csv('Inputs/PV_Output.csv', index_col=0)  # Import PV energy generation profile
...
SC_Energy = pd.read_csv('Inputs/SC_Output.csv', index_col=0) # Import solar collector energy generation profile
