...
def Total_Investment_Cost(model):
    return model.Total_Investment_Cost == model.RES_Investment_Cost + model.BESS_Investment_Cost + model.Generator_Investment_Cost + sum(model.Boiler_Investment_Cost[c] for c in model.classes)
...
def Fixed_Costs(model):
    return model.Fixed_Costs == model.RES_OM_Cost + model.BESS_OM_Cost + model.Generator_OM_Cost + sum(model.Boiler_OM_Cost[c] for c in model.classes)
...
def Electric_Energy_Balance(model,s,t):
    return model.Electric_Energy_Demand[s,t] == model.RES_Energy_Production[s,t] - model.BESS_Inflow[s,t] + model.BESS_Outflow[s,t] + model.Generator_Energy_Production[s,t] + model.Lost_Load_EE[s,t] - model.Electric_Curtailment[s,t]
