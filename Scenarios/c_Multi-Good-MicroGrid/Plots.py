...
def ElectricLoadCurves(instance):
    ...
    pylab.savefig('Results/Plots/ElectricLoadCurves.png', bbox_inches='tight', dpi=PlotResolution)
...
def ThermalLoadCurves(instance):
    ...
    for c in Thermal_Energy_Demand.columns:
        ...
        axs[0,0].legend(loc='upper left', fontsize=14, facecolor='white')
    
        i += 1
        
    pylab.savefig('Results/Plots/ThermalLoadCurves.png', bbox_inches='tight', dpi=PlotResolution)
...
def ThermalDispatch(instance,TimeSeries):
    ...
    for c in range(1,nC+1):
        ...
        x_Plot = np.arange(len(y_ElResProd))
        y_Stacked = [y_ElResProd,
                     y_LostLoad,
                     y_Curtailment]
