...
def Total_Investment_Cost(model):
    return model.Total_Investment_Cost == model.RES_Investment_Cost + model.BESS_Investment_Cost + model.Generator_Investment_Cost + sum(model.Electric_Resistance_Investment_Cost[c] for c in model.classes)
...
def Fixed_Costs(model):
    return model.Fixed_Costs == model.RES_OM_Cost + model.BESS_OM_Cost + model.Generator_OM_Cost + sum(model.Electric_Resistance_OM_Cost[c] for c in model.classes)
...
def Variable_Costs(model,s):
    return model.Variable_Costs[s] == model.Scenario_Lost_Load_Cost_EE[s] + model.Total_Diesel_Cost[s] + sum(model.Scenario_Lost_Load_Cost_Th[s,c] for c in model.classes)
...
def Thermal_Energy_Balance(model,s,c,t):
     return  model.Thermal_Energy_Demand[s,c,t] == model.Electric_Resistance_Energy_Production[s,c,t] - model.Thermal_Energy_Curtailment[s,c,t] + model.Lost_Load_Th[s,c,t]
