...
def Thermal_Energy_Balance(model,i,c,t): # Thermal energy balance
     ...
     :param model: Pyomo model as defined in the Model_creation library.
     '''
     return  model.Thermal_Energy_Demand[i,c,t] == model.Resistance_Thermal_Energy[i,c,t] - model.Thermal_Energy_Curtailment[i,c,t] + model.Lost_Load_Th[i,c,t]
...
def Scenario_Net_Present_Cost(model, i): 
    ...
    :param model: Pyomo model as defined in the Model_creation library.
    '''            
    return model.Scenario_Net_Present_Cost[i] == model.Initial_Investment_Cost + model.Operation_Maintenance_Cost + model.Total_Financial_Cost + model.Battery_Replacement_Cost + model.Scenario_Lost_Load_Cost_EE[i] + model.Scenario_Lost_Load_Cost_Th[i] + model.Total_Diesel_Cost[i]
...
def Initial_Investment_Cost(model):
    ...
    :param model: Pyomo model as defined in the Model_creation library.
    '''    
    return model.Initial_Investment_Cost == (model.RES_Units*model.RES_Investment_Cost*model.RES_Nominal_Capacity + model.Battery_Nominal_Capacity*model.Battery_Invesment_Cost + model.Generator_Nominal_Capacity*model.Generator_Invesment_Cost + model.Resistance_Financial_Cost )*(1-model.Percentage_Funded) 
...
def Loan_Financial_Cost(model): 
    ...
    :param model: Pyomo model as defined in the Model_creation library.
    '''
    return model.Loan_Financial_Cost == ((model.RES_Units*model.RES_Investment_Cost*model.RES_Nominal_Capacity + model.Battery_Nominal_Capacity*model.Battery_Invesment_Cost + model.Generator_Nominal_Capacity*model.Generator_Invesment_Cost + model.Resistance_Financial_Cost)*model.Percentage_Funded*model.Interest_Rate_Loan)/(1-((1+model.Interest_Rate_Loan)**(-model.Years)))
