...
def BESS_State_of_Charge(model,s,t):
    if t==1:
        return model.BESS_State_of_Charge[s,t] == model.BESS_Nominal_Capacity - model.BESS_Outflow[s,t]/model.BESS_Discharge_Efficiency + model.BESS_Inflow[s,t]/60*model.BESS_Charge_Efficiency
    if t>1:  
        return model.BESS_State_of_Charge[s,t] == model.BESS_State_of_Charge[s,t-1] - model.BESS_Outflow[s,t]/model.BESS_Discharge_Efficiency + model.BESS_Inflow[s,t]/60*model.BESS_Charge_Efficiency    
...
def Max_Power_BESS_Charge(model): 
    return model.Maximum_BESS_Charge_Power == model.BESS_Nominal_Capacity/(model.BESS_Maximum_Charge_Time/model.Delta_Time)

def Max_Power_BESS_Discharge(model):
    return model.Maximum_BESS_Discharge_Power == model.BESS_Nominal_Capacity/(model.BESS_Maximum_Discharge_Time/model.Delta_Time)
